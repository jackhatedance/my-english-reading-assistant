#### toc




#### 我本地有一个英文文本文件，想要利用这个插件的功能显示生词含义，怎么办？
* 方法一：编辑本地网页。用Word/WPS等编辑器，把内容复制进去，再把文件另存为html格式。浏览器可以打开本地html文件。
* 方法二：使用在线编辑器，比如[stackedit](https://stackedit.io/app)。把英语文本粘贴到左侧的编辑区，在右侧的预览区就可以正常阅读。

#### 支持哪些格式的电子书？
支持常见的电子书格式。理论上，只要是网页版的电子书阅读器，都是支持的。大家可以自行搜索。

已知的一些网页版电子书阅读器：
|网址|格式|说明|
|--|--|--|
| [flowoss](https://app.flowoss.com) | epub||
| [mozilla pdf viewer](https://mozilla.github.io/pdf.js/web/viewer.html) | PDF | 点击右上角“>>”按钮，Open。|

#### 英文原著，外刊电子书哪里找？
已知的一些方法：
* 花点小钱去某宝请人“代找英文书”
* 在知乎上搜“外刊在哪里找”
* zlib
* TG

#### 为什么有些PDF格式电子书无法显示单词含义？
因为有些PDF是由扫描图片组成的，往往文件也比较大。此类PDF不支持显示单词含义的功能。

#### 为什么阅读PDF格式电子书时鼠标点击单词没有弹出窗口？
因为Mozilla的PDF阅读器有点特别。解决方法:
* 点击单词上方的词典含义1次
* 点击单词2次（不是双击）

#### 为什么我的网页加载变慢了
因为本软件需要处理当前页面里的每一个单词，为其查询含义。网页内容越多，处理时间越长。处理特别大的网页（如外刊PDF，不分章节的epub等）时，需要更长的处理时间。但是这个处理时间与阅读时间相比，肯定是很短的。没有启用此软件功能的网页几乎不受影响。

#### 手机上可以使用吗？
在上下班途中看小说，或者在公园，在沙发上躺着看小说，都是很惬意的事情。手机浏览器不支持扩展。但是可以在手机和平板上通过远程桌面App连接电脑实现。在Flowoss网站上看epub电子书需要启用屏幕键盘翻页。亲测iPad和安卓手机连接windows11效果不错。在家里局域网环境实现这个不难。但是出门在外想要连接到家里的电脑上，需要借助动态域名技术实现。

![{width=200}](screenshots/cn/phone-remote-desktop.jpg "")

#### 为什么有些词典无法提取结构化数据？
结构化数据指词典里的关键数据，比如音标，词性，简明含义，短语等。大部分Mdict词典的内容是Html格式的，且样式不统一。提取结构化数据需要开发对应的解析器才行。请选用已知[支持导入的Mdict词典](#词典哪里找)，或者反馈给开发者。


#### 词典哪里找？
* [mdict.org](https://mdx.mdict.org)
* [freemdict.com](https://freemdict.com/)

支持导入的Mdict词典列表(陆续添加中)
|名称|发布时间|更新时间|大小|发音|图片|支持度|下载链接|
|-|-|-|-|-|-|-|-|
|新牛津英汉双解大词典（第2版）|2015||30MB|无|无|良好|[mdict.org](https://mdx.mdict.org/Recommend/%E6%96%B0%E7%89%9B%E6%B4%A5%E8%8B%B1%E6%B1%89%E5%8F%8C%E8%A7%A3%E5%A4%A7%E8%AF%8D%E5%85%B8%EF%BC%88%E7%AC%AC2%E7%89%88%EF%BC%89.zip)|
|牛津高阶英汉双解词典（第9版）|2019||197MB|无|有|良好|[mdict.org](https://mdx.mdict.org/Recommend/%E7%89%9B%E6%B4%A5%E9%AB%98%E9%98%B6%E8%8B%B1%E6%B1%89%E5%8F%8C%E8%A7%A3%E8%AF%8D%E5%85%B8%EF%BC%88%E7%AC%AC9%E7%89%88%EF%BC%89-%20%E5%B8%A6%E9%AB%98%E6%B8%85%E7%89%88%E5%9B%BE%E7%89%87.zip)|
|韦氏高阶英汉双解词典|2018|2019|200MB|有|有|良好|[mdict.org](https://mdx.mdict.org/Recommend/%E9%9F%A6%E6%B0%8F%E9%AB%98%E9%98%B6%E8%8B%B1%E6%B1%89%E5%8F%8C%E8%A7%A3%E8%AF%8D%E5%85%B8.zip)|
|英汉大词典|2019|2019|27MB|无|无|良好|[mdict.org](https://mdx.mdict.org/Recommend/%E8%8B%B1%E6%B1%89%E5%A4%A7.zip)|

字典下载网站与本软件无关，这里仅提供链接。欢迎推荐优秀的词典。

*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*