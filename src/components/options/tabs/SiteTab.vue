...
import { PARTIAL_TOKENIZATION_MODE_ON, PARTIAL_TOKENIZATION_MODE_OFF, PARTIAL_TOKENIZATION_MODE_AUTO } from '../../../partial-tokenization-mode.js'
...
const selectedPartialTokenizationMode = ref('none');
...
async function onChangePartialTokenization() {
  const partialTokenization = {
    mode: selectedPartialTokenizationMode.value,
  };
  let newOptions = { partialTokenization };
  await updateOptions(newOptions);
}

function updatePartialTokenization(options){
  selectedPartialTokenizationMode.value = options.partialTokenization.mode;
}
...
const init = async () => {
  ...
  updatePartialTokenization(options);
};
...
<template>
  ...
  <div class="sections">
    ...
    <div class="section">
      <div class="label">
        <h3>{{ t('options_site_partial_tokenization_label') }}</h3>        
      </div>
      <div class="input">
        <div>
