
export const DICTIONAY_TYPE_SYSTEM = 'system';
export const DICTIONAY_TYPE_USER = 'user';

export const DICTIONARY_FORMAT_TEXT = 'text';
export const DICTIONARY_FORMAT_MDICT = 'mdict';

export const DICTIONARY_ENTRY_TYPE_LINK = 'link';

export const DICTIONARY_DEFINITION_TYPE_LINK = 'link';
export const DICTIONARY_DEFINITION_TYPE_FORM = 'form';

export const DICTIONARY_DEFINITION_TYPE_FORM_SINGULAR = 'singular';
export const DICTIONARY_DEFINITION_TYPE_FORM_PLURAL = 'plural';
export const DICTIONARY_DEFINITION_TYPE_FORM_THIRD_PERSON_SINGULAR_PRESENT = 'third person singular present';
export const DICTIONARY_DEFINITION_TYPE_FORM_PAST_OR_PAST_PARTICIPLE = 'past or past participle';
export const DICTIONARY_DEFINITION_TYPE_FORM_PRESENT_PARTICIPLE = 'present pariciple';

export const MAX_SUBDEFINITION_NUMBER = 30;
export const PARSER_OPTION_MAX_SUBDEFINITION_NUMBER = 'maxSubdefinitionNumber';
export const PARSER_OPTION_DEDUPLICATE_SUBDEFINITIONS = 'deduplicateSubdefinitions';
export const PARSER_OPTION_DEBUG_PRINT_SELECTOR_FIND = 'debugPrintSelectorFind';
export const PARSER_OPTION_ALL_UPPER_CASE_ENTRY_POLICY = 'allUpperCaseEntryPolicy';
export const PARSER_OPTION_GENERATE_DEFINITION_TEXT = 'generateDefinitionText';

export const ALL_UPPER_CASE_ENTRY_POLICY_LOWER_CASE = 'lowerCase';

export const DICTIONARY_INDEX_STATUS_NOT_SUPPORT = 'NOT_SUPPORT';
export const DICTIONARY_INDEX_STATUS_OK = 'OK';
export const DICTIONARY_INDEX_STATUS_VERSION_MISMATCH = 'VERSION_MISMATCH';
export const DICTIONARY_INDEX_STATUS_NONE = 'NONE';