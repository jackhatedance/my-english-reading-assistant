...
describe('TextDefinitionUtils test', function () {
  ...
  describe('parse', function () {
    ...
    it('splitIntoDefinitionGroupsV2 bombings SysLarge', async function () {
      let parseResult = splitIntoDefinitionGroups('n. [军] 轰炸, [军] 投弹;v. 轰击；引爆炸弹（bomb的ing形式）', '2');
    ...
    it('splitIntoDefinitionGroupsV2 no word class brigand', async function () {
      let parseResult = splitIntoDefinitionGroups("n. 土匪, 强盗;[法] 土匪, 盗贼", '2');
      
      assert.equal(parseResult.length, 1);
      assert.equal(parseResult[0], "n. 土匪, 强盗;[法] 土匪, 盗贼");
    ...
    it('splitIntoDefinitionGroupsV2 vowed multiple wordclass', async function () {
      let parseResult = splitIntoDefinitionGroups('vt.& vi. 起誓, 发誓（vow的过去式与过去分词形式）', '2');
    ...
    it('splitIntoDefinitionGroupsV2 squeezed', async function () {
      let parseResult = splitIntoDefinitionGroups('squeeze（挤压）的过去式与过去分词', '2');
    ...
    it('splitIntoDefinitionGroupsV2 will sysSmall', async function () {
      let parseResult = splitIntoDefinitionGroups("n. 意志, 决心, 意愿; 意向, 干劲, 遗嘱; vt. 用意志的力量驱使, 决意, 愿意, 立遗嘱; vi. 下决心, 愿意; aux. 将, 愿意, 必须", '2');
    ...
    it("splitIntoDefinitionGroupsV2 here's sysLarge", async function () {
      let parseResult = splitIntoDefinitionGroups("abbr. (=here is. Here's to you)! 干杯;[网络] 这里有；这里是；用", '2');
      
      assert.equal(parseResult.length, 1);
      assert.equal(parseResult[0], "abbr. (=here is. Here's to you)! 干杯;[网络] 这里有；这里是；用");
